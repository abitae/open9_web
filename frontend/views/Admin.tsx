    ...
    <section className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
      <div className="w-full max-w-md bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-100">
        ...
        <input
          ...
          aria-describedby={error ? 'login-error' : undefined}
        />
        {error && <p id="login-error" className="text-red-500 text-sm font-bold mb-4" role="alert">{error}</p>}
        <button
          onClick={handleLogin}
          disabled={loading}
...
export const AdminDashboard: React.FC<{ 
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ type: 'project' | 'logo'; id: number } | null>(null);
  ...
  return (
    <section className="pt-32 pb-24 bg-slate-50 min-h-screen">
            ...
            <div className="lg:col-span-2 space-y-6">
              {clientLogos.map(logo => (
                <div key={logo.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-all">
                  ...
                  <button
                    onClick={() => setConfirmDelete({ type: 'logo', id: logo.id })}
                    className="p-4 text-red-500 hover:bg-red-50 rounded-2xl transition-all"
                    aria-label={`Eliminar logo ${logo.name}`}
                  >
                    <Trash2 size={24} />
                  </button>
                </div>
              ...
              {projects.length === 0 ? (
                ...
                projects.map(p => (
                  <div key={p.id} className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-all">
                    ...
                    <button
                      onClick={() => setConfirmDelete({ type: 'project', id: p.id })}
      ...
      <ConfirmModal
        ...
        title={confirmDelete?.type === 'project' ? '¿Eliminar este proyecto?' : '¿Eliminar este logo?'}
        message={confirmDelete?.type === 'project' ? 'El proyecto se quitará del portafolio público.' : 'El logo se quitará del carrusel de la página principal.'}
        confirmLabel="Eliminar"
        onConfirm={() => {
          if (confirmDelete) {
            if (confirmDelete.type === 'project') onDeleteProject(confirmDelete.id);
            else onDeleteClientLogo(confirmDelete.id);
